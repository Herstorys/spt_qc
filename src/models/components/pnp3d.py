...
class PnP3D(nn.Module):
    ...
    def forward(self, xyz, features, k):
        ...
        # 适配SPT数据格式：从NAG格式转换
        if xyz.dim() == 2:  # [N, 3] -> [1, 3, N]
            xyz = xyz.unsqueeze(0).transpose(1, 2)
        if features.dim() == 2:  # [N, C] -> [1, C, N]
            features = features.unsqueeze(0).transpose(1, 2)

        # 验证维度匹配
        batch_size, _, num_points_xyz = xyz.size()
        batch_size_feat, feat_dim, num_points_feat = features.size()

        assert num_points_xyz == num_points_feat, \
            f"Point count mismatch: xyz has {num_points_xyz}, features has {num_points_feat}"
        assert batch_size == batch_size_feat, \
            f"Batch size mismatch: xyz has {batch_size}, features has {batch_size_feat}"
        ...
        if f_out.size(0) == 1:
            f_out = f_out.squeeze(0).transpose(0, 1)  # [1, C, N] -> [N, C]
